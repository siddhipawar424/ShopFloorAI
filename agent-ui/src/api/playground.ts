...
export const getAllPlaygroundSessionsAPI = async (
  ...
  try {
    ...
    const response = await fetch(
      `${APIRoutes.GetPlaygroundSessions(base, agentId)}${query}`,
      {
        method: 'GET'
      }
    )
    ...
    const data = await response.json()
    return data.map((session: SessionEntry) => ({
      ...session,
      created_at: session.created_at ?? session.updated_at ?? 0
    }))
  } catch {
    return []
  }
...
export const getPlaygroundSessionAPI = async (
  ...
  const query = userId ? `?user_id=${encodeURIComponent(userId)}` : ''
  const response = await fetch(
    `${APIRoutes.GetPlaygroundSession(base, agentId, sessionId)}${query}`,
    {
      method: 'GET'
    }
  )
...
export const deletePlaygroundSessionAPI = async (
  ...
  const query = userId ? `?user_id=${encodeURIComponent(userId)}` : ''
  const response = await fetch(
    `${APIRoutes.DeletePlaygroundSession(base, agentId, sessionId)}${query}`,
    {
      method: 'DELETE'
    }
  )
...
export const getPlaygroundTeamSessionsAPI = async (
  ...
  try {
    const query = userId ? `?user_id=${encodeURIComponent(userId)}` : ''
    const response = await fetch(
      `${APIRoutes.GetPlaygroundTeamSessions(base, teamId)}${query}`,
      {
        method: 'GET'
      }
    )
...
export const getPlaygroundTeamSessionAPI = async (
  ...
  const query = userId ? `?user_id=${encodeURIComponent(userId)}` : ''
  const response = await fetch(
    `${APIRoutes.GetPlaygroundTeamSession(base, teamId, sessionId)}${query}`,
    {
      method: 'GET'
    }
  )
...
export const deletePlaygroundTeamSessionAPI = async (
  ...
  const query = userId ? `?user_id=${encodeURIComponent(userId)}` : ''
  const response = await fetch(
    `${APIRoutes.DeletePlaygroundTeamSession(base, teamId, sessionId)}${query}`,
    {
      method: 'DELETE'
    }
  )
