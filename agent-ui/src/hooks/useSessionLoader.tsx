  ...
  const getSessions = useCallback(
    async ({ entityType, agentId, teamId }: LoaderArgs) => {
      ...
      try {
        ...
        const sessions =
          entityType === 'team'
            ? await getPlaygroundTeamSessionsAPI(selectedEndpoint, teamId!, user?.id?.toString())
            : await getAllPlaygroundSessionsAPI(selectedEndpoint, agentId!, user?.id?.toString())
