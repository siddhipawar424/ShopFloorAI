...
const useAIChatStreamHandler = () => {
  ...
  const refreshSessions = useCallback(async () => {
    ...
    await getSessions({
      entityType: agentId ? 'agent' : 'team',
      agentId,
      teamId
    })
  }, [agentId, teamId, getSessions])
  ...
  const handleStreamResponse = useCallback(
    async (input: string | FormData) => {
      ...
      if (selectedLanguage === 'hindi') {
        finalMessage = `[System Directive: Respond strictly in Hindi language (हिंदी). If the query is statistical or uses databases, explain the results in clear Hindi text. Do not reply in English.] ${rawMessage}`
      } else if (selectedLanguage === 'english') {
        finalMessage = `[System Directive: Respond strictly in English language. Do not reply in Hindi.] ${rawMessage}`
      }
      ...
      // Add clean, un-prepended message to the UI
      addMessage({
        role: 'user',
        content: rawMessage,
      ...
      // Prepend directive for the backend API
