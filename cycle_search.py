...
load_dotenv()
DB_HOST = os.getenv("CYCLETIME_DB_HOST")
DB_NAME = os.getenv("CYCLETIME_DB_NAME")
DB_USER = os.getenv("CYCLETIME_DB_USER")
DB_PASSWORD = os.getenv("CYCLETIME_DB_PASSWORD")
DB_PORT = int(os.getenv("CYCLETIME_DB_PORT", 5432))
...
def get_table_columns(table_name="ft_cycletime_analysis"):
...
def get_sample_row(table_name="ft_cycletime_analysis"):
    conn = get_db_connection()
...
def generate_sql(user_prompt, table_name="ft_cycletime_analysis"):
...
"""
    ...
    '''    # FIX: Replace hallucinated 'data' column with correct JSON column if needed
    if "data ->" in sql_text:
        # Default to time_series_analysis (most common JSON column)
        sql_text = sql_text.replace("data ->", "time_series_analysis ->")'''
...
def cycle_time_tool(user_prompt: str):
    """
    Fully dynamic, natural-language cycle_time_tool tool.
